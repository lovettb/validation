...
use Lovettb\Exceptions\ValidatorContextException;
...
abstract class ValidatorService implements MessageProviderInterface {
	...
	function __construct($attributes = null, $context = null, $mergeDefault = true, Request $request) {
	...
	public function errorsArray() {
		...
		foreach ($this->attributes as $item) {
			if ($this->errors->has($item)) {
				$errors[$item] = $this->errors->first($item);
			}
		}
		return $errors;
	}
